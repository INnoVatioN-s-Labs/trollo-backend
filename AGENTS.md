...
-   **테스트 강제**: 모든 기능 개발/수정 시 JUnit 기반 단위 테스트(Unit Test)를 반드시 함께 작성합니다.
-   **검증 강제**: 코드 변경 후 반드시 `./gradlew test`를 실행해 통과를 확인해야 하며, 실패 상태에서는 작업 완료로 간주하지 않습니다.
-   **범위 규칙**: 신규 기능은 정상 케이스와 핵심 실패 케이스(예외/검증 실패)를 최소 1개 이상 포함해 테스트합니다.
